...
install_shadowsocks(){
    ...
    clear
mv_brand_header "INSTALAR SHADOWSOCKS"
    anim_init 5
    ...
    echo
    mv_deliv_header "$(trx '🎉 SHADOWSOCKS INSTALADO')" "$(trx 'Proxy SOCKS5 cifrado')"
    echo
    mv_deliv_sec "$(trx '📲 DATOS DE CONEXIÓN')"
    mv_dcard_top
    mv_dcard_row "🌍" "$(trx 'IP')" "$VPS_IP"
    mv_dcard_row "🚀" "$(trx 'Puerto')" "$SS_PORT"
    mv_dcard_row "🔑" "$(trx 'Clave')" "$(mv_tick "$SS_PASSWORD")"
    mv_dcard_row "🔐" "$(trx 'Método')" "$SS_METHOD"
    mv_dcard_bot
    echo
    mv_deliv_sec "$(trx '🔗 ENLACE SS')"
    echo -e "${GREEN}$(mv_tick "$SS_URL")${RESET}"
    echo
    mv_deliv_sec "$(trx '✅ INSTRUCCIONES')"
    echo -e "${GREEN}✔ $(trx 'Importa la URL en tu app Shadowsocks.')${RESET}"
    echo -e "${GREEN}✔ $(trx 'El cliente debe aceptar el certificado si usa TLS.')${RESET}"
    echo
    mv_deliv_pie
...
reconfig_shadowsocks(){
    ...
    echo
    mv_deliv_header "$(trx '🔄 NUEVA CLAVE SHADOWSOCKS')" "$(trx 'Credenciales actualizadas')"
    echo
    mv_deliv_sec "$(trx '📲 DATOS ACTUALIZADOS')"
    mv_dcard_top
    mv_dcard_row "🌍" "$(trx 'IP')" "$VPS_IP"
    mv_dcard_row "🚀" "$(trx 'Puerto')" "$SS_PORT"
    mv_dcard_row "🔑" "$(trx 'Nueva Clave')" "$(mv_tick "$SS_PASSWORD")"
    mv_dcard_row "🔐" "$(trx 'Método')" "$SS_METHOD"
    mv_dcard_bot
    echo
    VPS_IP=$(hostname -I | awk '{print $1}')
    SS_URL="ss://$(printf '%s' "$SS_METHOD:$SS_PASSWORD@$VPS_IP:$SS_PORT" | base64 -w0)#MoviVIP"
    mv_deliv_sec "$(trx '🔗 NUEVO ENLACE SS')"
    echo -e "${GREEN}$(mv_tick "$SS_URL")${RESET}"
    echo
    mv_deliv_pie
    read -n1 -r -p "$(trx 'Presione cualquier tecla para continuar...')"
...
show_info() {
    ...
    echo
    mv_deliv_header "$(trx '📱 DATOS SHADOWSOCKS')" "$(trx 'Proxy SOCKS5 cifrado')"
    echo
    mv_deliv_sec "$(trx '📲 DATOS DE CONEXIÓN')"
    mv_dcard_top
    mv_dcard_row "🌍" "$(trx 'IP')" "$VPS_IP"
