...
clear
mv_brand_header "INSTALANDO XRAY CORE"
    anim_init 8
    ...
    # FIX v7.3.3: sin cron automático de --check-limits (la verificación es manual y no destructiva)
...
xray_dias_restantes() {
    ...
    RESTANTES=$(( ( $(date -d "$VENCE" +%s) - $(date +%s) + 86399 ) / 86400 ))
...
show_vmess_user() {
    ...
    LINK="vmess://$(generate_vmess_link "$USER" "$KEY")"
    ...
    echo
    mv_deliv_header "$(trx '✅ CUENTA VMess CREADA')" "$(trx 'Xray · Protocolo VMess')"
    echo
    mv_deliv_sec "$(trx '📲 CONFIGURACIÓN DE CONEXIÓN')"
    mv_dcard_top
    mv_dcard_row "👤" "$(trx 'Usuario')" "$USER"
    mv_dcard_row "🆔" "UUID" "$KEY"
    mv_dcard_row "🌐" "$(trx 'Dominio')" "$DOMAIN"
    mv_dcard_row "🔒" "$(trx 'Puerto')" "$PORT"
    mv_dcard_row "🛡" "$(trx 'Seguridad')" "$SEC"
    mv_dcard_row "📡" "$(trx 'Network')" "WebSocket"
    mv_dcard_row "📂" "Path" "/vmess"
    mv_dcard_row "💾" "$(trx 'Consumo')" "$GBU GB / $MAXGB GB"
    mv_dcard_row "🔗" "$(trx 'Conexiones')" "máx $MAXCONN simultáneas (0=ilimitado)"
    mv_dcard_row "📅" "$(trx 'Días')" "$DREST restantes / $MAXDIAS"
    if grep -F "$USER=" "$XRAY_SUSPEND_FILE" >/dev/null 2>&1; then
        mv_dcard_row "⛔" "$(trx 'Estado')" "$(trx 'SUSPENDIDO')"
    fi
    mv_dcard_bot
    echo
    mv_deliv_sec "$(trx '🔗 ENLACE VMESS')"
    echo -e "${GREEN}$(mv_tick "$LINK")${RESET}"
    echo
    mv_deliv_pie
    read -n1 -r -p "$(trx 'Presione cualquier tecla para continuar...')"
...
create_vmess_account() {
    ...
    if declare -F mv_ask_duracion >/dev/null 2>&1; then
        if mv_ask_duracion 0; then
            NOW_TS=$(date +%s)
            DIF_TS=$(( DUR_TS - NOW_TS ))
            if [[ "$DIF_TS" -gt 0 ]]; then
                NEW_DAYS=$(( (DIF_TS + 86399) / 86400 ))
            else
                NEW_DAYS=0
            fi
        else
...
if [[ -z "$DOMAIN" ]]; then
    ...
    echo
    local SEC="TLS"
    [[ "$NEW_PORT" == "80" || "$NEW_PORT" == "8080" ]] && SEC="SIN TLS"

    mv_deliv_header "$(trx '🎉 CUENTA VMess CREADA')" "$(trx 'Xray · Protocolo VMess')"
    echo
    mv_deliv_sec "$(trx '📲 CONFIGURACIÓN DE CONEXIÓN')"
    mv_dcard_top
    mv_dcard_row "👤" "$(trx 'Usuario')" "$VMESS_USER"
    mv_dcard_row "🆔" "UUID" "$VMESS_UUID"
    mv_dcard_row "🌐" "$(trx 'Dominio')" "$DOMAIN"
    mv_dcard_row "🔒" "$(trx 'Puerto')" "$NEW_PORT"
    mv_dcard_row "🛡" "$(trx 'Seguridad')" "$SEC"
    mv_dcard_row "📡" "$(trx 'Network')" "WebSocket"
    mv_dcard_row "📂" "Path" "/vmess"
    mv_dcard_bot
    ...
    echo -e "${GREEN}✔ $(trx 'La cuenta está lista para usar.')${RESET}"
    echo -e "${GREEN}✔ $(trx 'Comparta el enlace con el cliente.')${RESET}"
    echo
    mv_deliv_pie
...
suspend_xray_user() {
    ...
    # FIX v7.3.3: SUSPENSIÓN NO DESTRUCTIVA.
    #   Antes se ELIMINABA el cliente del config.json (jq select != email) →
    #   "la xray me elimina los clientes". Ahora el cliente PERMANECE en el
    #   config.json; el evento queda registrado en xray_suspendidos.conf y
    #   xray_cortes.log para que el administrador decida qué hacer.
    #   reactivate_xray_user sigue funcionando (mismo secreto guardado).

    echo "$(date '+%Y-%m-%d %H:%M:%S') SUSPENDIDO(soft) $USER ($IDX) por $REASON — cliente NO eliminado del config" >> "$XRAY_CORTES_LOG"
...
create_vless_account() {
    ...
    mv_deliv_header "$(trx '🎉 CUENTA VLESS CREADA')" "$(trx 'Xray · Protocolo VLESS')"
    echo
    mv_deliv_sec "$(trx '📲 CONFIGURACIÓN DE CONEXIÓN')"
    mv_dcard_top
    mv_dcard_row "👤" "$(trx 'Usuario')" "$USERNAME"
    mv_dcard_row "🆔" "UUID" "$UUID"
    mv_dcard_row "🌐" "$(trx 'Dominio')" "$DOMAIN"
    mv_dcard_row "🔒" "$(trx 'Puerto')" "$NEW_PORT"
    mv_dcard_row "🛡" "$(trx 'Seguridad')" "$SEC"
    mv_dcard_row "📡" "$(trx 'Network')" "WebSocket"
    ...
    mv_dcard_bot
    echo
    mv_deliv_sec "$(trx '🔗 ENLACE VLESS')"
    echo -e "${GREEN}$(mv_tick "$VLESS_LINK")${RESET}"
    echo
    mv_deliv_sec "$(trx '✅ INSTRUCCIONES')"
    echo -e "${GREEN}✔ $(trx 'La cuenta está lista para usar.')${RESET}"
    echo -e "${GREEN}✔ $(trx 'Comparta el enlace con el cliente.')${RESET}"
...
create_trojan_account() {
    ...
    mv_deliv_header "$(trx '🎉 CUENTA Trojan CREADA')" "$(trx 'Xray · Protocolo Trojan')"
    echo
    mv_deliv_sec "$(trx '📲 CONFIGURACIÓN DE CONEXIÓN')"
    mv_dcard_top
    mv_dcard_row "👤" "$(trx 'Usuario')" "$USERNAME"
    mv_dcard_row "🔑" "Password" "$PASS"
    mv_dcard_row "🌐" "$(trx 'Dominio')" "$DOMAIN"
    mv_dcard_row "🔒" "$(trx 'Puerto')" "$NEW_PORT"
    mv_dcard_row "🛡" "$(trx 'Seguridad')" "$SEC"
    mv_dcard_row "📡" "$(trx 'Network')" "WebSocket"
    ...
    mv_dcard_bot
    echo
    mv_deliv_sec "$(trx '🔗 ENLACE TROJAN')"
    echo -e "${GREEN}$(mv_tick "$TROJAN_LINK")${RESET}"
    echo
    mv_deliv_sec "$(trx '✅ INSTRUCCIONES')"
    echo -e "${GREEN}✔ $(trx 'La cuenta está lista para usar.')${RESET}"
    echo -e "${GREEN}✔ $(trx 'Comparta el enlace con el cliente.')${RESET}"
...
show_vless_user() {
    ...
    LINK="vless://$(generate_vless_link "$USER" "$KEY")"
...
show_trojan_user() {
    ...
    LINK="trojan://$(generate_trojan_link "$USER" "$KEY")"
...
# 2) v7.3.3: NO instala cron automático (suspension soft/manual)
...
if [[ "$1" == "--ensure-api" ]]; then
    ...
    sleep 1
    if systemctl is-active --quiet xray; then
        echo -e "${GREEN}  ✅ API de stats activada (v7.3.3: sin cron automático, suspensión no destructiva).${RESET}"
