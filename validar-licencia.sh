...
# ── Design System premium (si el panel está instalado) ──
if [[ -f "/etc/movivip/lib/ui.sh" ]]; then
    source "/etc/movivip/lib/ui.sh" 2>/dev/null || true
fi
# ── Fallback: cabecera clásica si no existe el panel (instalación fría) ──
if ! declare -F mv_brand_header >/dev/null 2>&1; then
    mv_brand_header() {
        local T="${1:-MOVIVIP}"
        clear 2>/dev/null
        echo -e "\e[1;36m═══════════════════════════════════════════════════════════\e[0m"
        echo -e "  \e[1;97m◆ ${T}\e[0m"
        echo -e "\e[1;36m═══════════════════════════════════════════════════════════\e[0m"
    }
fi

mv_brand_header "VALIDACIÓN DE LICENCIA"
