...
nav_pick() {
    ...
    _np_item(){
        ...
        pad=$(( COLW - 7 - $(printf '%s' "$LBL" | wc -L) ))   # FIX V2: ancho VISUAL real (wc -L, emojis=2 celdas); (( pad < 0 )) && pad=0
        if (( i == SEL )); then
    ...
    _np_render(){
        ...
        for ((r=0; r<ROWS; r++)); do
            ...
            if (( COLS == 2 && ri <= N )); then
                # Separador vertical entre columnas (alineado, forma una línea │ continua)
                printf >&2 " \033[1;90m│\033[0m"
                if [[ "$MODE" == "clean" ]]; then
    ...
    while :; do
        ...
        if [[ "$K" == $'\x1b' ]]; then
            ...
            if [[ -n "$BUF" ]]; then
                if [[ "$BUF" =~ ^[0-9]+$ ]]; then
                    if (( BUF == 0 )); then OUT=0
                    elif (( BUF >= 1 && BUF <= N )); then OUT=$(( 10#$BUF )); SEL=$(( 10#$BUF ))
                    else BUF=""; continue
                    fi
                else
                    BUF=""
            ...
            (( ${#BUF} < 3 )) && BUF+="$K"
            # FIX v7.5: la sonda de 0.02s SE COMIA EL ENTER => pedias [15]+ENTER
            # y el menu quedaba colgado ("no responde"). Entrada numerica canonica:
            # los digitos se acumulan y solo el ENTER (rama de arriba) ejecuta,
            # normalizando con $((10#$BUF)) para que "01".."09" tambien funcionen.
        fi    done
