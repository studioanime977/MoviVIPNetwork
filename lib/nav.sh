...
# mv_vislen — columnas VISIBLES de una cadena en el terminal
# Ignora códigos ANSI · emoji/CJK = 2 columnas · box/geo/dingbats/ASCII = 1
mv_vislen() {
    local s="$1" LC_ALL=C
    s=$(printf '%s' "$s" | sed 's/\x1b\[[0-9;?]*[A-Za-z]//g')
    local n=${#s} i=0 w=0 b b2 b3 cp
    while (( i < n )); do
        b=$(printf '%d' "'${s:i:1}")
        if (( b < 0xC0 )); then
            (( w++, i++ ))
            ...
            if (( cp >= 0xFE00 && cp <= 0xFE0F )); then
                :                    # selectores de variación = 0 cols
            elif (( (cp >= 0x2500 && cp <= 0x25FF) || (cp >= 0x2190 && cp <= 0x21FF) || (cp >= 0x2700 && cp <= 0x27BF) )); then
                (( w++ ))            # box/geométricos, flechas, dingbats = 1
            elif (( cp >= 0x2600 && cp <= 0x26FF )); then
                (( w += 2 ))         # símbolos/emojis (⚡☀☁…) = 2
            elif (( cp >= 0x2E80 )); then
                (( w += 2 ))         # CJK + emoji = 2
            else
                (( w++ ))
            ...
            (( w += 2, i += 4 ))     # 4 bytes (emoji) = 2 columnas
        fi
    done
    echo "$w"
}
...
# ── Header estándar para SUBMENÚS — PREMIUM v2.1 (logo 3D + marco) ──
# Con lib/ui.sh cargado usa el mismo marco premium del menú principal
movivip_sub_header() {
    local TITLE="$1"
    # Design System PREMIUM si está disponible
    if declare -F mv_brand_header >/dev/null 2>&1; then
        mv_brand_header "$TITLE"
    elif declare -F mv_center >/dev/null 2>&1; then
        mv_line_thin
...
# ============================================================
GOLD="${MV_GLD:-$GOLD}"; CYAN="${MV_CYN:-$CYAN}"; GRAY="${MV_DIM:-$GRAY}"; WHITE="${MV_WHT:-$WHITE}"; RESET="${MV_R:-$RESET}"
    ...
    # Anchos visuales pre-calculados (1 sola pasada; evita subprocess por keypress)
    local -a WID=()
    for (( i=1; i<=N; i++ )); do
        WID[i]=$(mv_vislen "${L[i-1]}")
    done

    # Renderiza UN item rellenando su columna completa
    _np_item(){
        local i=$1
        local LBL="${L[i-1]}" NUM pad VIS=${WID[i]:-0}
        if [[ "$LBL" == *↩* || "$LBL" == *Salir* ]]; then
            NUM="00"
        else
            NUM=$(printf '%02d' "$i")
        fi
        pad=$(( COLW - 8 - VIS )); (( pad < 0 )) && pad=0
        if (( i == SEL )); then
            (( pad -= 1 )); (( pad < 0 )) && pad=0
    ...
    while :; do
        ...
        if [[ "$K" == $'\x1b' ]]; then
            ...
            # FIX v5.8: entrada numerica fluida 1-3 digitos.
            # v5.7 drenaba el buffer 0.15s tras CADA digito: al escribir/paste
            # "15", el '5' y ENTER ya estaban en buffer y el drenaje se los
            # comia => menu colgado en [1] => opciones >=10 "no ejecutaban".
            (( ${#BUF} < 3 )) && BUF+="$K"
            # Anti-duplicado movil LIGERO: solo consume eco instantaneo (0.02s)
            # sin devorar la siguiente tecla humana.
            local _d=""
            if IFS= read -rsn1 -t 0.02 _d 2>/dev/null; then
                if [[ "$_d" =~ ^[0-9]$ ]]; then
                    (( ${#BUF} < 3 )) && BUF+="$_d"
                elif [[ -z "$_d" ]]; then
                    OUT="$BUF"
...
movivip_soporte_screen() {
    ...
    mv_brand_header "Soporte MoviVIP  [v${VERSION:-7.3}]" "Centro de Ayuda · Compatibilidad · Contactos"
    ...
    printf "   🌐 SlowDNS ${MV_DIM}[53·5300]${MV_R} ⚡ BadVPN ${MV_DIM}[7200·7300]${MV_R}  🚀 UDP Custom ${MV_DIM}[36712]${MV_R}\n"
    printf "   ☁️ Xray ${MV_DIM}[VLESS·VMess·Trojan WS/TLS/gRPC · 443]${MV_R}\n"
    printf "   🌀 Hysteria ${MV_DIM}[UDP]${MV_R}  🛡️ OpenVPN ${MV_DIM}[1194]${MV_R}  🛡️ ZiVPN ${MV_DIM}[UDP 5667]${MV_R}\n"
...
printf '\033[?9001l\033[?1004l' 2>/dev/null || true