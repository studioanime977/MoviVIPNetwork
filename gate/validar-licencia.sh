...
main() {
    ...
    # ═══════════════════════════════════════════════════════════════
    # KEY-IP BINDING — Verificar vínculo key-IP en activaciones
    # Si la key ya está vinculada a OTRA IP → RECHAZAR (salvo admin)
    # ══════════════════════════════════════════════════════════════
    check_key_ip_binding() {
        local key="$1" current_ip="$2"
        [[ -z "$key" || -z "$current_ip" ]] && return 0

        # Admin/superadmin pueden usar la key en cualquier IP
        local plan_tipo=$(echo "${plan:-}" | tr '[:upper:]' '[:lower:]')
        local tipo_tipo=$(echo "${cliente:-}" | tr '[:upper:]' '[:lower:]')
        case "$plan_tipo" in admin|superadmin|super|empresarial|vitalicio) return 0 ;; esac
        case "$tipo_tipo" in admin|superadmin|super) return 0 ;; esac

        local FB_URL="https://${FB_BASE}/${FB_ACTIVACIONES}/${key}.json"
        local EXISTING=$(curl -fsSL --max-time 10 "$FB_URL" 2>/dev/null)
        if [[ -n "$EXISTING" && "$EXISTING" != "null" ]]; then
            local BOUND_IPS=$(echo "$EXISTING" | grep -o '"ip":"[^"]*"' | cut -d'"' -f4 | sort -u)
            for ip in $BOUND_IPS; do
                if [[ "$ip" != "$current_ip" && "$ip" != "unknown" ]]; then
                    log_err "✖ KEY VINCULADA A OTRA IP: $ip"
                    log_err "  Esta key solo puede usarse en la IP donde se activó primero."
                    log_warn "  Contacta a soporte si cambiaste de VPS."
                    return 1
                fi
            done
        fi
    ...
    # Ejecutar binding check ANTES de registrar activación
    local current_ip
    current_ip=$(get_ip)
    check_key_ip_binding "$KEY" "$current_ip" || return 1
