...
# ═══════════════════════════════════════════════════════════════
# AWS CLI v2 INSTALLATION (para CloudFront Distribution)
# ═══════════════════════════════════════════════════════════════
install_aws_cli() {
    if command -v aws >/dev/null 2>&1; then
        echo -e "      ${GREEN}✔${RESET} AWS CLI ya instalado: $(aws --version 2>&1 | head -1)"
        return 0
    fi
    ...
    case "$ARCH" in
        ...
        *) echo -e "      ${RED}✖${RESET} Arquitectura no soportada: $ARCH"; return 1 ;;
    esac
    cd /tmp
    curl -s "$AWS_URL" -o "awscliv2.zip" || { echo -e "      ${RED}✖${RESET} Error descargando AWS CLI"; return 1; }
    unzip -q -o awscliv2.zip || { echo -e "      ${RED}✖${RESET} Error descomprimiendo"; return 1; }
    ./aws/install --update 2>/dev/null || ./aws/install 2>/dev/null
    rm -rf aws awscliv2.zip
    cd - >/dev/null
    if command -v aws >/dev/null 2>&1; then
        echo -e "      ${GREEN}✔${RESET} AWS CLI instalado: $(aws --version 2>&1 | head -1)"
        return 0
    else
        echo -e "      ${RED}✖${RESET} Fallo instalando AWS CLI"
        return 1
    fi
}

# ═══════════════════════════════════════════════════════════════
# PLAN VALIDATION — Premium/Vitalicio/Admin only
# ═══════════════════════════════════════════════════════════════
PLAN_CHECK_OK=()
check_plan_premium() {
    ...
    case "$plan" in
        premium|vitalicio|admin|super|empresarial) PLAN_CHECK_OK=(1 "$FP_PLAN" "$FP_TIPO"); return 0 ;;
    esac
    case "$tipo" in
        admin|superadmin|super|empresarial) PLAN_CHECK_OK=(1 "$FP_PLAN" "$FP_TIPO"); return 0 ;;
    esac
    PLAN_CHECK_OK=(0 "${FP_PLAN:-}" "${FP_TIPO:-}")
    return 1
}

# ═══════════════════════════════════════════════════════════════
# CLOUDFRONT DISTRIBUTION CREATION
# Solo para planes ADMIN / PREMIUM / VITALICIO (NO bronce/free)
# ═══════════════════════════════════════════════════════════════
create_cloudfront_distribution() {
    local DOMAIN="$1"
    [[ -z "$DOMAIN" ]] && { echo -e "      ${RED}✖${RESET} Dominio requerido"; return 1; }
    check_plan_premium
    if [[ "${PLAN_CHECK_OK[0]}" != "1" ]]; then
        echo -e "      ${RED}✖${RESET} CloudFront solo para planes: ${YELLOW}PREMIUM / VITALICIO / ADMIN${RESET}"
        echo -e "      ${GRAY}   Plan actual: ${PLAN_CHECK_OK[1]:-desconocido} / ${PLAN_CHECK_OK[2]:-desconocido}${RESET}"
        return 1
    fi
    install_aws_cli || return 1
    if [[ -z "${AWS_ACCESS_KEY_ID:-}" || -z "${AWS_SECRET_ACCESS_KEY:-}" ]]; then
        if [[ -f "$CONFIG" ]]; then source "$CONFIG"; fi
        export AWS_ACCESS_KEY_ID="${CLOUDFRONT_ACCESS_KEY_ID:-${AWS_ACCESS_KEY_ID:-}}"
        export AWS_SECRET_ACCESS_KEY="${CLOUDFRONT_SECRET_ACCESS_KEY:-${AWS_SECRET_ACCESS_KEY:-}}"
        export AWS_DEFAULT_REGION="${CLOUDFRONT_REGION:-${AWS_DEFAULT_REGION:-us-east-1}}"
    fi
    if [[ -z "$AWS_ACCESS_KEY_ID" || -z "$AWS_SECRET_ACCESS_KEY" ]]; then
        echo -e "      ${RED}✖${RESET} Credenciales AWS no configuradas"
        echo -e "      ${GRAY}   Configura CLOUDFRONT_ACCESS_KEY_ID y CLOUDFRONT_SECRET_ACCESS_KEY en config.conf${RESET}"
        return 1
    fi
    step "Creando CloudFront Distribution para $DOMAIN..."
    local ORIGIN_IP=$(curl -s ifconfig.me 2>/dev/null || hostname -I | awk '{print $1}')
    local CF_CONFIG=$(cat <<CFEOF
{
    "CallerReference": "movivip-$(date +%s)",
    "Comment": "MoviVIP Network - $DOMAIN",
    "DefaultRootObject": "",
    "Origins": {"Quantity": 1, "Items": [{"Id": "movivip-origin", "DomainName": "$ORIGIN_IP", "OriginPath": "", "CustomHeaders": {"Quantity": 0}, "CustomOriginConfig": {"HTTPPort": 80, "HTTPSPort": 443, "OriginProtocolPolicy": "https-only", "OriginSslProtocols": {"Quantity": 1, "Items": ["TLSv1.2"]}, "OriginReadTimeout": 30, "OriginKeepaliveTimeout": 5}]},
    "DefaultCacheBehavior": {"TargetOriginId": "movivip-origin", "ViewerProtocolPolicy": "redirect-to-https", "AllowedMethods": {"Quantity": 7, "Items": ["GET","HEAD","OPTIONS","PUT","POST","PATCH","DELETE"], "CachedMethods": {"Quantity": 2, "Items": ["GET","HEAD"]}}, "Compress": true, "MinTTL": 0, "DefaultTTL": 86400, "MaxTTL": 31536000, "ForwardedValues": {"QueryString": true, "Cookies": {"Forward": "all"}, "Headers": {"Quantity": 0}}, "SmoothStreaming": false, "LambdaFunctionAssociations": {"Quantity": 0}},
    "CacheBehaviors": {"Quantity": 0}, "CustomErrorResponses": {"Quantity": 0}, "Logging": {"Enabled": false}, "PriceClass": "PriceClass_All", "Enabled": true,
...
CFEOF
    ...
    local RESULT
    RESULT=$(aws cloudfront create-distribution --distribution-config "$CF_CONFIG" --region "${AWS_DEFAULT_REGION:-us-east-1}" 2>&1)
    local RC=$?
    if [[ $RC -eq 0 ]]; then
        local DIST_ID=$(echo "$RESULT" | grep -o '"Id": "[^"]*"' | head -1 | cut -d'"' -f4)
        local DIST_DOMAIN=$(echo "$RESULT" | grep -o '"DomainName": "[^"]*"' | head -1 | cut -d'"' -f4)
        local DIST_STATUS=$(echo "$RESULT" | grep -o '"Status": "[^"]*"' | head -1 | cut -d'"' -f4)
        echo -e "      ${GREEN}✔${RESET} CloudFront Distribution creada"
        echo -e "      ${WHITE}   Distribution ID: ${CYAN}$DIST_ID${RESET}"
        echo -e "      ${WHITE}   Domain Name: ${CYAN}$DIST_DOMAIN${RESET}"
        echo -e "      ${WHITE}   Status: ${YELLOW}$DIST_STATUS${RESET}"
        echo -e "      ${GRAY}   Nota: La distribución tarda 5-15 min en desplegarse globalmente${RESET}"
        if grep -q "^CLOUDFRONT_DISTRIBUTION_ID=" "$CONFIG" 2>/dev/null; then sed -i "s|^CLOUDFRONT_DISTRIBUTION_ID=.*|CLOUDFRONT_DISTRIBUTION_ID=$DIST_ID|" "$CONFIG"; else echo "CLOUDFRONT_DISTRIBUTION_ID=$DIST_ID" >> "$CONFIG"; fi
        if grep -q "^CLOUDFRONT_DISTRIBUTION_DOMAIN=" "$CONFIG" 2>/dev/null; then sed -i "s|^CLOUDFRONT_DISTRIBUTION_DOMAIN=.*|CLOUDFRONT_DISTRIBUTION_DOMAIN=$DIST_DOMAIN|" "$CONFIG"; else echo "CLOUDFRONT_DISTRIBUTION_DOMAIN=$DIST_DOMAIN" >> "$CONFIG"; fi
        return 0
    else
        echo -e "      ${RED}✖${RESET} Error creando CloudFront Distribution"
        echo -e "      ${GRAY}$RESULT${RESET}"
        return 1
    fi
}
cloudfront_menu() {
    if [[ -z "${CLOUDFRONT_DOMAIN:-}" ]]; then echo -e "      ${YELLOW}⚠${RESET} No hay dominio CloudFront configurado"; read -rp "$(echo -e "${GREEN}Ingrese dominio CloudFront: ${RESET}")" CLOUDFRONT_DOMAIN; [[ -z "$CLOUDFRONT_DOMAIN" ]] && return 1; fi
    create_cloudfront_distribution "$CLOUDFRONT_DOMAIN"
    read -n1 -s -r -p "$(echo -e "${GRAY}Presione una tecla...${RESET}")"
}
...
if [[ -t 0 ]]; then
    ...
    read -rp "$(echo -e "${GREEN}AWS Access Key ID (para CloudFront):${RESET} ")" CLOUDFRONT_ACCESS_KEY_ID
    CLOUDFRONT_ACCESS_KEY_ID="${CLOUDFRONT_ACCESS_KEY_ID:-}"
    read -rp "$(echo -e "${GREEN}AWS Secret Access Key:${RESET} ")" CLOUDFRONT_SECRET_ACCESS_KEY
    CLOUDFRONT_SECRET_ACCESS_KEY="${CLOUDFRONT_SECRET_ACCESS_KEY:-}"
    read -rp "$(echo -e "${GREEN}AWS Region [us-east-1]:${RESET} ")" CLOUDFRONT_REGION
...
step "Configurando banner SSH..."
    ...
    # ═══════════════════════════════════════════════════════════════
    # BANNER SSH LOGIN (/etc/profile.d/MoviVIP-banner.sh) — HTML v3.1
    # ═══════════════════════════════════════════════════════════════
    ...
    case "${FP_TIPO:-${TIPO:-}}" in admin|superadmin|super) _SELLO_OK=0 ;; esac
    case "${FP_PLAN:-${PLAN:-}}" in super|vitalicio|empresarial) _SELLO_OK=0 ;; esac

    if [[ "$_SELLO_OK" == "1" ]]; then
        _BANNER_SEAL='<font color="#00ffff"><small><i>🛡SISTEMA PROTEGIDO POR MOVIVIP NETWORK🛡</i></small></font>'
    else
        _BANNER_SEAL=''
    fi

    case "${PLAN:-}" in
        premium|vitalicio|admin|super|empresarial) _SERVER_TYPE="<font color='#FFD700'><big> ▶ SERVER PREMIUM ◀</big></font>" ;;
        *) _SERVER_TYPE="<font color='#00ff00'><big> ▶ SERVER GRATUITO ◀ ¡Si lo compraste, te estafaron!</big></font>" ;;
    esac
...
<font color='#ffffff'><big>🛡 ⚔️ ${BRAND_SLOGAN:-ULTRA PERFORMANCE & MAXIMUM SPEED} ⚔️ 🛡</big></font><br><br>
<font color='#ffff00'>📢 Canal: ${TG_CHANNEL:-t.me/MoviVIPNetwork}</font><br>
<font color='#ffff00'>📢 Grupo: ${TG_GROUP:-t.me/MoviVIPNet}</font><br>
<font color='#29b6f6'>════════════════════════════</font><br><br>
<font color='#ffffff'><big>🤝 ⚡COLABORACIÓN / COMUNIDAD SOCIA ⚡ 🤝</big></font><br><br>
<font color='#ffff00'>📢 Canal: ${TG_CHANNEL:-t.me/MoviVIPNetwork}</font><br>
<font color='#ffff00'>📢 Grupo: ${TG_GROUP:-t.me/MoviVIPNet}</font><br>
<font color='#00ffff'>🌐 𝕎𝔼𝔹: ${BRAND_WEB:-https://movivip-network.web.app}</font><br><br>
<font color='#00ffff'>👤 Soporte: ${SUPPORT_LINK:-@MoviVIP}</font><br><br>
...
[[ -n "$_BANNER_GROUP" ]]    && _BANNER_DYNAMIC+="<font color='#ffff00'>📢 Grupo: ${_BANNER_GROUP}</font><br>\n"
    ...
    # ════════════════════════════════════════════════════════════════
    # BANNER SSH CONNECTION (/etc/issue.net) — HTML v3.1
    # Sello permanente salvo admin (validado EN VIVO contra Firebase)
    # ═══════════════════════════════════════════════════════════════
    ...
    [[ -z "$FP_TIPO" && -f "$BASE/licencia.conf" ]] && source "$BASE/licencia.conf" 2>/dev/null
    case "${FP_TIPO:-${TIPO:-}}" in admin|superadmin|super) _ISSUE_SELLO_OK=0 ;; esac
    case "${FP_PLAN:-${PLAN:-}}" in super|vitalicio|empresarial) _ISSUE_SELLO_OK=0 ;; esac

    if [[ "$_ISSUE_SELLO_OK" == "1" ]]; then
        _ISSUE_SEAL='<font color="#00ffff"><small><i>🛡SISTEMA PROTEGIDO POR MOVIVIP NETWORK🛡</i></small></font>'
    else
        _ISSUE_SEAL=''
    fi
