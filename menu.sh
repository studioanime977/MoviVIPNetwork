...
mv_panel_bot
printf "   ${GRAY}RAM${RESET} %b ${WHITE}%s%%${RESET}   ${GRAY}CPU${RESET} %b ${WHITE}%s%%${RESET}\n" \
    "$RAM_BAR" "$RAM_USE" "$CPU_BAR" "$CPU_USE"
...
21)
    ...
    if (( WANT_SSH )); then
        bash "$BASE/usuarios/account.sh" add_ssh_only "$NU" "$NP" "$ND" "$NL" "$NC"
    fi

    # 2) Xray / V2Ray (cliente propio + expiración y límite por usuario)
    if (( WANT_XRAY )); then
        if [[ -f /usr/local/etc/xray/config.json ]]; then
            bash "$BASE/usuarios/account.sh" xray_add_full "$NU" "$ND" "$NL"
        else
    ...
    if (( WANT_ZIV )); then
        if [[ -f /etc/zivpn/config.json ]]; then
            bash "$BASE/usuarios/account.sh" zipvpn_add "$NP" "$ND" "$NZG"
        else
...
0)
    ...
    mv_line_thin
    mv_center "${MV_ORA}｡ﾟﾟ･｡･ﾟﾟ｡ ${MV_R}${MV_CYN}⋆｡˚⊱ ${MV_GLD}${MV_BLD}M O V I V I P${MV_R}${MV_CYN} ⊰˚｡⋆${MV_R} ${MV_ORA}｡ﾟﾟ･｡･ﾟﾟ｡${MV_R}"
    echo ""
    exit 0
;;
