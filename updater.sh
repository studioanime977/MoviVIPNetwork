...
#=========================================================
#   MOVIVIP NETWORK â€” ACTUALIZADOR PREMIUM v5.0
...
ROW(){ printf "${CYAN}â•‘${RESET} %-56s${CYAN}â•‘${RESET}\n" "$1"; }
ROWC(){ printf "${CYAN}â•‘${RESET} %b%*s${CYAN}â•‘${RESET}\n" "$1" $(( 56 - $(echo -ne "$1" | sed 's/\x1b\[[0-9;]*m//g' | wc -c) )) ""; }
...
check_license() {
    ...
    # Verificar expiraciÃ³n (expira=0 = vitalicia)
    ...
    # ValidaciÃ³n online contra Firebase (fail-open para updater local)
    ...
    if [[ -n "$FB_DATA" ]]; then
        ...
        # Verificar expiraciÃ³n de Firebase
...
clear
TOP
printf "${CYAN}â•‘${RESET}  ${GOLD}ðŸ›¡ï¸  MoviVIP Network${RESET}  ${WHITE}ACTUALIZADOR v${VERSION:-5.0}${RESET}${CYAN}             â•‘${RESET}\n"
printf "${CYAN}â•‘${RESET}  ${GRAY}movivip-network.web.app${RESET}  ${GRAY}Â·${RESET}  ${WHITE}${PROTO_LIVE:-Ãšltima versiÃ³n}${RESET}${CYAN}                â•‘${RESET}\n"
MID
echo ""

# Verificar licencia
if ! check_license; then
    ROWC "${RED}âœ— LICENCIA NO VÃLIDA O VENCIDA${RESET}"
    ROWC "${GRAY}El updater requiere licencia activa${RESET}"
    echo ""
    ROWC "${WHITE}VersiÃ³n actual: ${GOLD}$(cat "$BASE/version.txt" 2>/dev/null || echo '?')${RESET}"
    echo ""
...
printf "${CYAN}â•‘${RESET} ${GOLD}[1/6]${RESET} Verificando conexiÃ³n...${CYAN}%*sâ•‘${RESET}\n" $(( W - 30 )) ""

if ! curl -fsSL --max-time 5 https://github.com &>/dev/null; then
    ROWC "${RED}âœ— Sin conexiÃ³n a internet${RESET}"
    BOT
    exit 1
fi
ROWC "${GREEN}âœ“ ConexiÃ³n establecida${RESET}"

#==============================
# [2] VERIFICAR VERSIÃ“N
#==============================

printf "${CYAN}â•‘${RESET} ${GOLD}[2/6]${RESET} Verificando versiÃ³n...${CYAN}%*sâ•‘${RESET}\n" $(( W - 27 )) ""
...
if [[ -z "$REMOTE_VER" ]]; then
    ROWC "${RED}âœ— No se pudo verificar versiÃ³n remota${RESET}"
    BOT
    exit 1
fi
...
if [[ "$VERSION_CHANGED" == "false" && "$COMMIT_CHANGED" == "false" ]]; then
    ROWC "${GREEN}âœ“ v${LOCAL_VER} â€” Ya estÃ¡s actualizado${RESET}"
    BOT
    echo ""
    read -rp "$(echo -e "${CYAN}   Enter para volver${RESET}")" _
...
if [[ "$VERSION_CHANGED" == "true" ]]; then
    ROWC "${WHITE}Local: ${GOLD}v${LOCAL_VER}${RESET}  â†’  Remota: ${GREEN}v${REMOTE_VER}${RESET}"
else
    ROWC "${WHITE}VersiÃ³n: ${GOLD}v${LOCAL_VER}${RESET} (sin cambios) â€” ${GREEN}Hay cambios en el cÃ³digo${RESET}"
fi
...
printf "${CYAN}â•‘${RESET} ${GOLD}[3/6]${RESET} Respaldando scripts...${CYAN}%*sâ•‘${RESET}\n" $(( W - 27 )) ""

mkdir -p "$BACKUP_DIR"
cp -r "$SCRIPTS_DIR"/* "$BACKUP_DIR/" 2>/dev/null
BACKUP_SIZE=$(du -sh "$BACKUP_DIR" 2>/dev/null | awk '{print $1}')
ROWC "${GREEN}âœ“ Respaldo (${BACKUP_SIZE:-?})${RESET}"

#==============================
# [4] DESCARGAR
#==============================

printf "${CYAN}â•‘${RESET} ${GOLD}[4/6]${RESET} Descargando v${GREEN}${REMOTE_VER}${RESET}...${CYAN}%*sâ•‘${RESET}\n" $(( W - 30 - ${#REMOTE_VER} )) ""

rm -rf "$TEMP_DIR"
git clone --depth 1 "$REPO" "$TEMP_DIR" 2>/dev/null

if [[ ! -d "$TEMP_DIR" ]]; then
    ROWC "${RED}âœ— Error de descarga â€” restaurando respaldo${RESET}"
...
if [[ -z "$SCRIPTS_SRC" ]]; then
    ROWC "${RED}âœ— Scripts no encontrados en repositorio${RESET}"
    rm -rf "$TEMP_DIR"
    BOT
    exit 1
fi
ROWC "${GREEN}âœ“ Descarga completa${RESET}"

#==============================
# [5] ACTUALIZAR
#==============================

printf "${CYAN}â•‘${RESET} ${GOLD}[5/6]${RESET} Actualizando...${CYAN}%*sâ•‘${RESET}\n" $(( W - 21 )) ""
...
# Fix CRLF from Windows â€” TODOS los .sh del sistema
...
ROWC "${GREEN}âœ“ ${UPDATED} mÃ³dulos actualizados${RESET}"

#==============================
# [6] IPTABLES GAMING
#==============================

printf "${CYAN}â•‘${RESET} ${GOLD}[6/6]${RESET} Configurando iptables gaming...${CYAN}%*sâ•‘${RESET}\n" $(( W - 33 )) ""
...
ROWC "${GREEN}âœ“ iptables configurado${RESET}"
...
printf "${CYAN}â•‘${RESET}  ${GREEN}âœ“ ACTUALIZACIÃ“N COMPLETADA${RESET}                            ${CYAN}â•‘${RESET}\n"
printf "${CYAN}â•‘${RESET}                                                          ${CYAN}â•‘${RESET}\n"
printf "${CYAN}â•‘${RESET}  ${GRAY}VersiÃ³n:${RESET}  ${GOLD}v${LOCAL_VER}${RESET} â†’ ${GREEN}v${REMOTE_VER}${RESET}${CYAN}%*sâ•‘${RESET}\n" $(( W - 23 - ${#LOCAL_VER} - ${#REMOTE_VER} )) ""
printf "${CYAN}â•‘${RESET}  ${GRAY}MÃ³dulos:${RESET}  ${WHITE}${UPDATED}${RESET}${CYAN}%*sâ•‘${RESET}\n" $(( W - 14 - ${#UPDATED} )) ""
printf "${CYAN}â•‘${RESET}  ${GRAY}Respaldo:${RESET} ${WHITE}${BACKUP_SIZE:-?}${RESET}${CYAN}%*sâ•‘${RESET}\n" $(( W - 17 - ${#BACKUP_SIZE} )) ""
printf "${CYAN}â•‘${RESET}                                                          ${CYAN}â•‘${RESET}\n"
MID
printf "${CYAN}â•‘${RESET}  ${GRAY}Restaurar:${RESET} ${WHITE}cp -r ${BACKUP_DIR}/* /etc/movivip/${RESET}${CYAN}%*sâ•‘${RESET}\n" $(( W - 8 - ${#BACKUP_DIR} )) ""
printf "${CYAN}â•‘${RESET}                                                          ${CYAN}â•‘${RESET}\n"
BOT
