...
# Detecta expiración corrupta (año > 2100)
fecha_ok() {
    local f="$1"
    [[ "$f" =~ ([0-9]{4}) ]] || return 1
    [[ "${BASH_REMATCH[1]}" -ge 2000 && "${BASH_REMATCH[1]}" -le 2100 ]]
}
...
[[ -f "$BASE/lib/duracion.sh" ]] && source "$BASE/lib/duracion.sh" 2>/dev/null || true
...
while read -r USER; do
    ...
    if fecha_ok "$FECHA"; then
        printf "${GREEN}[%02d]${WHITE} %-18s ${GRAY}%s${RESET}\n" "$i" "$USER" "$FECHA"
    else
        printf "${GREEN}[%02d]${WHITE} %-18s ${RED}%s ⚠${RESET}\n" "$i" "$USER" "$FECHA"
    fi
    LISTA[$i]="$USER"
    ((i++))
done <<< "$USERS"
...
human() {
    local B=$1
    # Sanitizar: quitar CR/LF/espacios y quedarse solo con dígitos
    B="${B//[$'\r\n\t ']/}"
    B="${B//[^0-9]/}"
    [[ -z "$B" ]] && B=0
...
if fecha_ok "$FECHA"; then
    ...
    echo -e "${WHITE} Expira          : ${RED}$FECHA ⚠ CORRUPTA${RESET}"
fi
...
echo -e "${RED}[6]${WHITE} ♻ Reactivar / Desbloquear cuenta"
echo -e "${GRAY}[0]${WHITE} Volver"
...
clear
mv_brand_header "♻ EDITAR / RENOVAR USUARIO SSH"
...
# ♻ Reactivar / Desbloquear cuenta
SUSP_CONF="$BASE/sistema/suspendidos.conf"
EXP_CONF="$BASE/sistema/expiraciones_exactas.conf"

clear
echo -e "${RED}╔══════════════════════════════════════════════════════╗${RESET}"
echo -e "${RED}║${YELLOW}   ♻ REACTIVAR / DESBLOQUEAR CUENTA: ${WHITE}$USER${RED}   ║${RESET}"
echo -e "${RED}╠══════════════════════════════════════════════════════╣${RESET}"
echo -e "${WHITE} Estado passwd  : ${GREEN}$(passwd -S "$USER" 2>/dev/null | awk '{print $2}')${RESET}"
echo -e "${WHITE} Expira actual  : ${GREEN}$FECHA${RESET}"
echo -e "${RED}╠══════════════════════════════════════════════════════╣${RESET}"
echo -e "${GREEN}  1)${WHITE} Reactivar con NUEVA DURACIÓN"
echo -e "${GREEN}  2)${WHITE} Reactivar SIN LÍMITE (expiración infinita)"
echo -e "${GREEN}  3)${WHITE} Solo desbloquear (mantener fecha)"
echo -e "${GREEN}  0)${WHITE} Volver"
echo
read -rp "$(echo -e "${GREEN}Opción: ${RESET}")" OPRE
...
passwd -u "$USER" >/dev/null 2>&1
chage -E "$DUR_FECHA_DIA" "$USER"

# Quitar de suspendidos si estaba y registrar la nueva expiración
grep -v "^${USER}|" "$SUSP_CONF" > "$SUSP_CONF.tmp" 2>/dev/null
mv "$SUSP_CONF.tmp" "$SUSP_CONF" 2>/dev/null
mv_clear_exp_exacta "$USER" "$EXP_CONF" 2>/dev/null
if [[ -n "$DUR_TS" ]]; then
    mv_save_exp_exacta "$USER" "$DUR_TS" "$EXP_CONF"
fi

echo
echo -e "${GREEN}✔ Usuario ${WHITE}$USER${GREEN} reactivado — expira: ${YELLOW}${DUR_FECHA_DIA}${RESET}"
sleep 3
;;
...
echo
echo -e "${GREEN}✔ Usuario ${WHITE}$USER${GREEN} reactivado ${YELLOW}SIN LÍMITE${RESET}"
sleep 3
;;
...
# Si la fecha está corrupta, pedir una correcta
if ! fecha_ok "$FECHA"; then
    echo
    echo -e "${RED}⚠ La fecha actual ($FECHA) es inválida.${RESET}"
    read -rp "$(echo -e "${GREEN}Días a partir de hoy [30]: ${RESET}")" DIAS_FIX
    [[ -z "$DIAS_FIX" ]] && DIAS_FIX=30
    NEW_F=$(date -d "+$DIAS_FIX days" +"%Y-%m-%d")
    chage -E "$NEW_F" "$USER"
    EXP_CONF="$BASE/sistema/expiraciones_exactas.conf"
    NEW_TS=$(date -d "$NEW_F" +%s)
    mv_save_exp_exacta "$USER" "$NEW_TS" "$EXP_CONF"
    FECHA="$NEW_F"
fi
...
echo
echo -e "${GREEN}✔ Usuario ${WHITE}$USER${GREEN} desbloqueado (expira: ${YELLOW}$FECHA${GREEN})${RESET}"
sleep 3
;;
