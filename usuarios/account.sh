...
#   add_ssh_only <user> <pass> <days> <limit> <consumo_bytes>         (SOLO cuenta Linux/SSH, sin Xray ni ZipVPN)
...
cmd_add_ssh() {
    ...
    mv_save_exp_exacta "$USER" "$FECHA_TS" 2>/dev/null
...
#==================================================
# SSH ONLY (sin Xray, sin ZipVPN)
# Cuenta Linux: OpenSSH + Dropbear + SSL/TLS + WS + Payload + SOCKS5 ...
#==================================================
cmd_add_ssh_only() {
    local USER="$1"
    local PASS="$2"
    ...
    # SIN Xray (no se toca xray) y SIN ZipVPN
    echo "OK:$USER:$PASS:$FECHA:$LIMITE:$CONSUMO_BYTES"
}
...
cmd_add() {
    ...
    cmd_xray_add "$USER"
    cmd_zipvpn_add "$PASS" "$DIAS" "$GB_ZIPVPN" "$HORAS" "$MINUTOS"
...
cmd_zipvpn_add() {
    ...
    if [[ "$DIAS" -gt 0 || "$HORAS" -gt 0 || "$MINUTOS" -gt 0 ]]; then
        EXP=$(date -d "+${DIAS} days +${HORAS} hours +${MINUTOS} minutes" +%s)
    else
        EXP=0
    fi
...
cmd_xray_add_full() {
    ...
    UUID=$(cat /proc/sys/kernel/random/uuid)
    EXP_DATE=$(date -d "+$DIAS days +$HORAS hours +$MINUTOS minutes" +"%Y-%m-%d")
    EXP_TS=$(date -d "+$DIAS days +$HORAS hours +$MINUTOS minutes" +%s)
...
case "$ACTION" in
    ...
    add_ssh)
        ...
        ;;
    add_ssh_only)
        cmd_add_ssh_only "$2" "$3" "$4" "$5" "$6" "$7" "$8"
        ;;
    delete)
        cmd_delete "$2" "$3"
        ;;
    ...
    *)
        ...
        echo "$(trx 'Uso: account.sh <add|add_ssh|add_ssh_only|delete|delete_ssh|list|get_config|zipvpn_add|zipvpn_remove|zipvpn_list|xray_add|xray_remove|xray_list|slowdns_key|set_limit|get_limit|zipvpn_set_limit>')"
        exit 1
        ;;
esac
