...
LBL=("${USER_ADD:-Crear Usuario}" "${USER_DELETE:-Eliminar}" "${USER_EDIT:-Editar/Renovar}" "${USER_LIST:-Lista de Usuarios}" "${USER_CONNECT:-Conectados}" "${USER_BANNER:-Banner SSH}" "${USER_BLOCK:-Bloquear}" "${USER_BACKUP:-Backup}" "${USER_REACT:-Reactivar Suspendidos ♻️}" "${USER_ADD_HWID:-Usuario HWID}" "${USER_LIST_HWID:-HWID List}" "${USER_CHANGE_HWID:-Cambiar HWID}" "${USER_BLOCK_HWID:-HWID Bloqueos}" "${USER_LIMIT_HWID:-HWID Cuota 📊}" "${USER_RENEW_HWID:-Renovar HWID ⏰}")
...
9) bash "$BASE/usuarios/reactivar.sh" ;;
10) bash "$BASE/usuarios/add_hwid.sh" ;;
11) bash "$BASE/usuarios/hwid_list.sh" ;;
12) bash "$BASE/usuarios/change_hwid.sh" ;;
13) bash "$BASE/usuarios/hwid_bloqueos.sh" ;;
14) bash "$BASE/usuarios/hwid_limite.sh" ;;
15) bash "$BASE/usuarios/hwid_renovar.sh" ;;
